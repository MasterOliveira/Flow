...
package org.flowplayer.controls.buttons {
	...
	public class AbstractSlider extends ConfigurableWidget {
		...
		protected function get valueFromScrubberPosVertical():Number {
			//#349 if we click to seek to zero the calculation includes dragger dimensions so is negative, correct this to zero
			//bound position between 0 and 100
			return clampPos(((mouseY - _dragger.height / 2) / (height - _dragger.height)) * 100);
		}
...
//            log.debug("onMouseOver");
		...
		private function updateCurrentPosFromDraggerVertical():void {
			_currentPos = (_dragger.y / (height - _dragger.height)) * 100;
		}
		...
		protected function get maxDragVertical():Number {
			return height - _dragger.height;
		}
